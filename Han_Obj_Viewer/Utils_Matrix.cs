//http://download.csdn.net/download/jian_5030624/1082723
...
namespace Han_Obj_Viewer
{
    public class Matrix
    {
        ...
        public bool ComputeEvJacobi(double[] dblEigenValue, Matrix mtxEigenVector, double eps)
        {
            ...
            Matrix mtxIn = new Matrix(this);

            if (!mtxEigenVector.Init(mtxIn.numColumns, mtxIn.numColumns))
                return false;

            for (i = 0; i <= mtxIn.numColumns - 1; i++)
            {
                mtxEigenVector.elements[i * mtxIn.numColumns + i] = 1.0;
                for (j = 0; j <= mtxIn.numColumns - 1; j++)
                    if (i != j)
                        mtxEigenVector.elements[i * mtxIn.numColumns + j] = 0.0;
            }
            ...
            for (i = 1; i <= mtxIn.numColumns - 1; i++)
            {
                for (j = 0; j <= i - 1; j++)
                {
                    d = mtxIn.elements[i * mtxIn.numColumns + j];
                    ff = ff + d * d;
                }
            }
            ...
            while (true)
            {
                for (i = 1; i <= mtxIn.numColumns - 1; i++)
                {
                    for (j = 0; j <= i - 1; j++)
                    {
                        d = Math.Abs(mtxIn.elements[i * mtxIn.numColumns + j]);
                        if (d > ff)
                        {
                            p = i;
                            q = j;

                            u = p * mtxIn.numColumns + q;
                            w = p * mtxIn.numColumns + p;
                            t = q * mtxIn.numColumns + p;
                            s = q * mtxIn.numColumns + q;
                            x = -mtxIn.elements[u];
                            y = (mtxIn.elements[s] - mtxIn.elements[w]) / 2.0;
                            omega = x / Math.Sqrt(x * x + y * y);
                            if (y < 0.0)
                                omega = -omega;
                            ...
                            cn = Math.Sqrt(1.0 - sn * sn);
                            fm = mtxIn.elements[w];
                            mtxIn.elements[w] = fm * cn * cn + mtxIn.elements[s] * sn * sn + mtxIn.elements[u] * omega;
                            mtxIn.elements[s] = fm * sn * sn + mtxIn.elements[s] * cn * cn - mtxIn.elements[u] * omega;
                            mtxIn.elements[u] = 0.0;
                            mtxIn.elements[t] = 0.0;

                            for (j = 0; j <= mtxIn.numColumns - 1; j++)
                            {
                                if ((j != p) && (j != q))
                                {
                                    u = p * mtxIn.numColumns + j;
                                    w = q * mtxIn.numColumns + j;
                                    fm = mtxIn.elements[u];
                                    mtxIn.elements[u] = fm * cn + mtxIn.elements[w] * sn;
                                    mtxIn.elements[w] = -fm * sn + mtxIn.elements[w] * cn;
                                }
                            }
                            ...
                            for (i = 0; i <= mtxIn.numColumns - 1; i++)
                            {
                                if ((i != p) && (i != q))
                                {
                                    u = i * mtxIn.numColumns + p;
                                    w = i * mtxIn.numColumns + q;
                                    fm = mtxIn.elements[u];
                                    mtxIn.elements[u] = fm * cn + mtxIn.elements[w] * sn;
                                    mtxIn.elements[w] = -fm * sn + mtxIn.elements[w] * cn;
                                }
